"""
Session 1: Agent Memory Fundamentals - Demo Code (Supabase Edition)

This demo covers:
- Part 1: Agent without memory (the problem)
- Part 2: Adding memory with PostgresSaver (Supabase)
...
Prerequisites:
    pip install langchain langgraph langchain-openai langsmith langgraph-checkpoint-postgres psycopg[binary]
...
# Setup tables using a separate connection with autocommit (required for CREATE INDEX CONCURRENTLY)
...
def part2_agent_with_memory():
    """Demonstrates memory with PostgresSaver (Supabase) checkpointer."""
    
    print("=" * 60)
    print("PART 2: Agent WITH Memory (PostgresSaver/Supabase)")
    print("=" * 60)
    ...
    agent_with_memory = create_agent(
        ...
        tools=[],
        system_prompt="You are a helpful assistant. Remember user details.",
        checkpointer=checkpointer,  # Uses Supabase PostgresSaver
        name="memory_agent"
    )
    ...
    print(f"\nTurn 2 - User: What's my name and what do I love?")
    print(f"Turn 2 - Agent: {result2['messages'][-1].content}")
    print("\n[✓] Agent remembers! Memory persisted to Supabase.")
...
def part3_thread_isolation():
    ...
    # Create agent with Supabase-backed memory
    ...
    print("\n[✓] Threads are completely isolated! (Stored in Supabase)")
